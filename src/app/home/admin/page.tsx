...
function AdminPageContent() {
  ...
  // OWNER → branchId dari URL param. ADMIN → dari profile.
  ...
  const [newStaff,     setNewStaff]     = useState({ username: '', password: '', displayName: '', role: 'EMPLOYEE' as 'ADMIN' | 'EMPLOYEE' })
  ...
  // SETTINGS
  const [settingsForm, setSettingsForm] = useState({ store_name: '', store_address: '', store_social: '', footer_text: '' })

  // ── FETCH — semua filter by activeBranchId ──
  ...
  const fetchReport = useCallback(async () => {
    ...
    const { data: orders } = await supabase.from('orders')
      ...
      .eq('branch_id', activeBranchId).gte('created_at', fromISO).eq('status','COMPLETED')
    const { data: items } = await supabase.from('order_items')
      .select('menu_name,quantity,orders!inner(created_at,status,branch_id)')
      .eq('orders.branch_id', activeBranchId).gte('orders.created_at', fromISO).eq('orders.status','COMPLETED')
  ...
  const updateDisplayName = async (id:string,name:string) => {
  ...
  const handleSettingsSave = async () => {
    ...
    const {error} = await supabase.from('store_settings')
      .upsert({branch_id:activeBranchId,...settingsForm},{onConflict:'branch_id'})
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto w-full">
      ...
      {/* Tombol kembali ke owner page (kalau owner) */}
      {isOwner && (
      ...
      {tab==='staff' && (
        <div>
          ...
          {showAddStaff && (
            <div className="bg-gray-900 border border-gray-700 rounded-xl p-4 mb-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <input placeholder="Nama Tampil *" value={newStaff.displayName}
                  onChange={e=>setNewStaff({...newStaff,displayName:e.target.value})}
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500"/>
                <input type="password" placeholder="Password * (min 6 karakter)" value={newStaff.password}
                  onChange={e=>setNewStaff({...newStaff,password:e.target.value})}
                  className="bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-500"/>
                <div className="flex gap-2">
                  {(['EMPLOYEE','ADMIN'] as const).map(r=>(
                    <button key={r} type="button" onClick={()=>setNewStaff({...newStaff,role:r})}
                      className={`flex-1 py-2 rounded-lg text-xs font-bold transition-colors ${
                        newStaff.role===r?'bg-orange-600 text-white':'bg-gray-800 text-gray-400'}`}>
                      {r==='EMPLOYEE'?'👤 Pegawai':'🛡️ Admin'}
                    </button>
                  ))}
                </div>
              </div>
          ...
          <div className="space-y-2">
            ...
            {staff.length===0 && (
              <div className="text-center text-gray-600 py-8">Belum ada pegawai di cabang ini</div>
            )}
          </div>
        </div>
      )}
      ...
      {tab==='settings' && (
        <div>
          ...
          <div className="bg-gray-900 border border-gray-700 rounded-xl p-5 space-y-4 max-w-lg">
            {[
              ['Nama Toko','store_name','kasir-dapur'],
              ['Alamat','store_address','Jl. Contoh No. 1'],
              ['Sosial Media','store_social','@instagram'],
              ['Teks Penutup Struk','footer_text','Terima kasih!'],
            ].map(([label,key,placeholder])=>(
              <div key={key}>
                <label className="block text-xs font-semibold text-gray-400 mb-1.5">{label}</label>
                <input value={settingsForm[key as keyof typeof settingsForm]}
...
function StaffRow({staff,onRoleChange,onNameChange}:{
  staff:Profile, onRoleChange:(id:string,role:UserRole)=>void, onNameChange:(id:string,name:string)=>void
}) {
  ...
  return (
    <div className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex flex-col sm:flex-row gap-3 sm:items-center">
      <div className="flex-1">
        <div className="text-xs text-gray-500 mb-1 font-mono">@{staff.username||'(no username)'}</div>
        {editingName ? (
          <div className="flex gap-2">
            ...
            <button onClick={()=>{onNameChange(staff.id,name);setEditingName(false)}}
              className="px-3 py-1 bg-orange-600 rounded text-xs font-bold">Simpan</button>
            <button onClick={()=>{setName(staff.display_name);setEditingName(false)}}
              className="px-3 py-1 bg-gray-700 rounded text-xs">Batal</button>
          </div>
        ) : (
      ...
      <div className="flex gap-2">
        {(['OWNER','ADMIN','EMPLOYEE'] as UserRole[]).map(r=>(
          <button key={r} onClick={()=>onRoleChange(staff.id,r)}
            className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-colors ${
              staff.role===r
                ?r==='OWNER'?'bg-orange-600 text-white':r==='ADMIN'?'bg-purple-700 text-white':'bg-blue-700 text-white'
